...
Dispatch is adaptive: a phase with fewer than $32{,}768$ independent tasks runs inline on the calling thread, a fixed threshold chosen once for all workloads rather than tuned per graph.
...
Our first implementation spawned $T{-}1$ fresh operating-system threads per phase; on a 147K-edge graph at $B{=}1000$ this made per-batch latency grow almost linearly in $T$, from 0.53\,ms at $T{=}1$ to 30.4\,ms at $T{=}32$, because thread creation and teardown dominated the actual work.
...
Inline dispatch for small phases removed both effects: latency became thread-count invariant wherever the phases are small, and the pool engages exactly on the phases that are large enough to pay for it (on the dense-community graphs, insertion-heavy batches at $B{=}10$K produce regions approaching a hundred thousand edges, and the pool handles their recount).
