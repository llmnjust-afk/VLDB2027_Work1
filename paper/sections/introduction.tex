...
The model was introduced by Cohen~\cite{cohen2019trusses}, and today the trussness of every edge underlies community detection~\cite{conte2020truly}, truss-based and attributed community search~\cite{huang2016attribute,behrouz2022firmtruss}, structural diversity search~\cite{huang2020trussbased}, and temporal cohesive-subgraph analytics~\cite{lotito2020efficient,saryce2025powerful}.
...
On graphs whose dense part is a single large truss-connected component---the common case in practice, from email corpora to co-purchase networks, and the case whose update-time sensitivity is documented by Chen et al.~\cite{chen2021breaking}---every update's closure covers the entire component, so the total work of a batch is the component size times the batch size rather than the component size.
...
We implemented the algorithm in C++ with a work-stealing thread pool and validated it with a differential-testing harness that compares the maintained decomposition against a static reference implementation after every batch, on randomized graphs, streams with adversarial update profiles, and real-world datasets.
Experiments over eight public graphs (25K to 69M edges) and streams of 200K updates show that batching collapses the multi-round fixpoint behavior of naive batch designs into one peeling pass, that the maintained decomposition is exact on every workload we tested, and that a faithful sequential per-edge baseline charged for the same batched workloads is 42--345$\times$ slower per batch on the dense-community graphs, while our per-batch cost sits an order of magnitude below full recomputation on graphs whose dense structure is localized and at parity with recomputation where every batch's recount touches the dense core or a hub set.
...
\item an experimental evaluation over eight public graphs demonstrating that batching and the single-pass peel deliver maintenance costs an order of magnitude below full recomputation on community-structured graphs, and mapping the dense-core regime in which recomputation itself is the right baseline.
\end{itemize}
